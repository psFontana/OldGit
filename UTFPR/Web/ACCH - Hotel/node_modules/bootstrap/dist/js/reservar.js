document.getElementById("reserva").addEventListener("submit", (event) => {
  ...
  const valor = valoresQuartos[quarto] * pessoas;

  if (!nome || !telefone || !email || !comprovante || !quarto || !pessoas) {
  ...
  if (!telefoneRegex.test(telefone)) {
    ...
    feedback.innerText = "Por favor, insira um telefone válido no formato (XX) XXXXX-XXXX.";
    return;
  }
...
function atualizaValor() {
  if (document.querySelector('input[name="tipoQuarto"]:checked') != null && document.getElementById("quantidadePessoas") != null) {
    ...
    const valor = new Intl.NumberFormat('pt-BR', {
      style: 'currency',
      currency: 'BRL'
    }).format(valoresQuartos[quarto] * pessoas);

    if (isNaN(pessoas) || pessoas <= 0) {
      
      lbl.innerText = "Por favor, insira uma quantidade de pessoas válida." 
    }else{
      lbl.innerText = valor
    }
  }
